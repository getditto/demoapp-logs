...
private struct DittoLogConfig {
    ...
    static var logFileURL: URL! = {
        return Self.logsDirectory.appendingPathComponent(DittoLogConfig.logFileName)
    }()
    ...
    static var zippedLogsURL: URL! = {
        ...
        return directory.appendingPathComponent(DittoLogConfig.zippedLogFileName)
    }()
}

/// LogManager acts as a thin interface over our stored log files and may be
/// used to offer functionality to share zipped logs with an iOS share sheet.
struct LogManager {
    ...
    // MARK: - Initialization
    private init() { /* Private singleton constructor */ }
    ...
    public var logFileURL: URL? {
        ...
        do {
            try FileManager().createDirectory(at: DittoLogConfig.logsDirectory,
        ...
        return DittoLogConfig.logFileURL
    }
    ...
    public func createLogsZip() -> URL? {
        try? FileManager().removeItem(at: DittoLogConfig.zippedLogsURL)
        ...
        coordinator.coordinate(
            readingItemAt: DittoLogConfig.logsDirectory,
            options: [.forUploading], error: &nsError
        ) { tempURL in
            do {
                try FileManager().moveItem(at: tempURL, to: DittoLogConfig.zippedLogsURL)
            } catch let error {
        ...
        return DittoLogConfig.zippedLogsURL
    }
}
